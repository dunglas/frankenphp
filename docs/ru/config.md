...
Статический бинарник будет искать `Caddyfile` в директории, из которой он запущен.

PHP можно настроить [с помощью файла `php.ini`](https://www.php.net/manual/en/configuration.file.php).

По умолчанию PHP, поставляемый с Docker-образами, и тот, что включён в статический бинарник, будет искать файл `php.ini` в директории, где был запущен FrankenPHP, а также в `/usr/local/etc/php/`. Кроме того, будут загружены все файлы с расширением `.ini` из директории `/usr/local/etc/php/conf.d/`.
