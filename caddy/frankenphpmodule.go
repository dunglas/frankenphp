...
// parsePhpDirective unmarshals tokens from h into a new Middleware.
func parsePhpDirective(h httpcaddyfile.Helper) (caddyhttp.MiddlewareHandler, error) {
	m := &FrankenPHPModule{}
...
// parsePhpServerDirective parses the php_server directive, which has a similar syntax
...
//		file_server
//
// parsePhpServerDirective is freely inspired from the php_fastgci directive of the Caddy server (Apache License 2.0, Matthew Holt and The Caddy Authors)
func parsePhpServerDirective(h httpcaddyfile.Helper) ([]httpcaddyfile.ConfigValue, error) {
	if !h.Next() {
		return nil, h.ArgErr()
	}
...
func parsePhpWorkerDirective(h httpcaddyfile.Helper) ([]httpcaddyfile.ConfigValue, error) {
	...
	frankenphpModule := FrankenPHPModule{}
	fsrv := fileserver.FileServer{}
	disableFsrv := false
	...
	for dispenser.Next() {
		for dispenser.NextBlock(0) {
			...
			case "root":
				...
				frankenphpModule.Root = dispenser.Val()
				fsrv.Root = frankenphpModule.Root
				dispenser.DeleteN(2)
			case "file_server":
				args := dispenser.RemainingArgs()
				dispenser.DeleteN(len(args) + 1)
	...
	if frankenphp.EmbeddedAppPath != "" {
		if frankenphpModule.Root == "" {
			...
			fsrv.Root = frankenphpModule.Root
			rrs := false
			frankenphpModule.ResolveRootSymlink = &rrs
		} else if filepath.IsLocal(fsrv.Root) {
			frankenphpModule.Root = filepath.Join(frankenphp.EmbeddedAppPath, frankenphpModule.Root)
			fsrv.Root = frankenphpModule.Root
		}
	}
	...
	if disableFsrv {
		return []httpcaddyfile.ConfigValue{
			{
				Class: "route",
				Value: getIndexWorkerWithoutFileServer(frankenphpModule),
			},
		}, nil
	}
...
func getIndexWorkerWithoutFileServer(frankenphpModule FrankenPHPModule) caddyhttp.Subroute {
	return caddyhttp.Subroute{
		Routes: caddyhttp.RouteList{
			caddyhttp.Route{
				MatcherSetsRaw: []caddy.ModuleMap{},
